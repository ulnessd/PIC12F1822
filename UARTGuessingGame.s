...
startloop: ;this loops waiting for the start of the game
    BANKSEL PIR1
    ...
    BANKSEL PORTA
    movlw 0b01111111 ; Always start with 127 as the first guess
    movwf 0x020 ; guess register --- this will be updated with the current guess
    movlw 0b10000000 ; guess step size this will be halved each round
    movwf 0x021 ; guess step size register
...
mainloop: ;this loops will the game is in progress
    ...
    movf  0x020, 0 ;puts the current guess in the working register
    BANKSEL TXREG
    movwf TXREG ; transmits the current guess to the computer
    nop

waitloop: ;this loop waits for the "too high/too low" response
    BANKSEL PIR1
    btfss PIR1, 5    ; Check if RCIF is set
    goto waitloop       ; If not, keep checking
    
    BANKSEL PORTA
    lsrf 0x021, 1 ; the rotate right divides the step size by 2
    
    
    BANKSEL RCREG
    movf RCREG, 0 ; moves the high/low response into the working register
		    ; this is needed to clear  RCIF!!
 
    btfss RCREG, 0
    call raise_guess ; "Too low" branch 
    nop
    btfsc RCREG, 0
    call lower_guess ; "Too High" branch
    nop
    nop
...
raise_guess:
    BANKSEL PORTA
    movf 0x021, 0 ; this puts the current guess step size into the working register
    addwf 0x020, 1 ; this adds the working register to the current guess and stores as new current guess
    return
    
lower_guess:
    BANKSEL PORTA
    movf 0x021, 0 ; this puts the current guess step size into the working register
    subwf 0x020, 1 ;this subtracts W from the current guess and stores as the new guess
    return
