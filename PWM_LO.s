...
main:
    ...
    clrf OSCCON
    movlw 0b01110011; this sets the oscillator to 8 MHz see page 65 to change
    movwf OSCCON
    ...
    BANKSEL PR2 ; Set the PWM frequency (bank 0)
    movlw 0b1100101; gives a 19.61 kHz PWM frequency (T = 51 us) when clock set to 8 MHz
    movwf PR2
		 
    movlw 0b00000100; sets timer to on (bit 2) and prescaler to 1 (see page 197)
    movwf T2CON
        
    BANKSEL CCP1CON ; Set the duty cycle information (bank 5)
    movlw 0b00001100; bit 7-6 set to 00 means olny P1A active this is PIN2
		    ; bit 5-4 are the lsb for the duty cycle setting
